...
$(document).ready(function () {
        ...
        $("#mouvement_Categorie option:selected").each(function () {
          for (let i = 0; i < result.length; i++) {
            ...
            if (element.categorie == $(this).text()) {
              ...
              $("#stock_actuelle").show();
              var prod_dispo =
                "Le(s) stock(s) disponibles pour " +
                element.categorie +
                ' sont : <span id="valeur_dispo">' +
                element.valeur_dispo +
                "<b id='type_prod'style='color: red; display: none;'>" +
                element.type_order_short +
                "</b>";
              if (element.type_order_short === "FP") {
                var nombre_par_planche =
                  '</span> <br/> Le nombre de timbre par planche(TP/PL) est : <span id="tp_par_pl">' +
                  element.tpParPl +
                  "</span>";
                document.getElementById("stock_actuelle").innerHTML =
                  prod_dispo + nombre_par_planche;
              } else {
                document.getElementById("stock_actuelle").innerHTML =
                  prod_dispo;
  ...
  $("#mouvement_quantite").on("change", function () {
    ...
    if (valDispo < quantite && types == 2) {
      alert(
        "Veuillez saisir de quantite ou nombre de planche inferieur a la valeur disponible "
      );
